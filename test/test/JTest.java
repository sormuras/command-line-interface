  ...
  @interface Test {}
    ...
    record Runner(List<Event> events) implements Listener {
        ...
        void print(PrintStream out) {
            Class<?> in = null;
            int maxNameLength = events.stream().mapToInt(ev -> ev.test.getName().length()).max().orElse(0);
            for (Event e : events) {
                ...
                out.printf("  [%s] %-"+maxNameLength+"s  %4d ms%n", status, e.test.getName(), e.executionTime.toMillis() );
            }
            out.println("\n"+"=".repeat(maxNameLength+16));
            out.printf("  [ok] %3d   %"+maxNameLength+"d ms%n",
                    events.stream().filter(e -> e.error.isEmpty()).count(),
                    events.stream().filter(e -> e.error.isEmpty()).mapToLong(e -> e.executionTime.toMillis()).sum());
            out.printf("  [!!] %3d   %"+(maxNameLength)+"d ms%n",
                    events.stream().filter(e -> e.error.isPresent()).count(),
                    events.stream().filter(e -> e.error.isPresent()).mapToLong(e -> e.executionTime.toMillis()).sum());
        }

        void verify() {
            if (events.stream().anyMatch(e -> e.error().isPresent()))
                ...
                .forEach(
                        method -> {
                            ...
                            try {
                                ...
                                method.invoke(this);
                                listener.accept(new Event(method, Duration.ofMillis(currentTimeMillis()- before), Optional.empty() ));
                            } catch (InvocationTargetException ex) {
                                listener.accept(new Event(method, Duration.ofMillis(currentTimeMillis()- before), Optional.of(ex.getTargetException()) ));
                            } catch (Exception ex) {
                                listener.accept(new Event(method, Duration.ofMillis(currentTimeMillis()- before), Optional.of(ex) ));
                            }
                        });
    }

...
public class JarTests implements TestRunner {
    ...
    record JarOptions(
            @Name({"-c", "--create"})
            boolean create,
            @Name({"-i", "--generate-index"})
            Optional<String> generateIndex,
            @Name({"-t", "--list"})
            boolean list,
            @Name({"-u", "--update"})
            boolean update,
            @Name({"-x", "--extract"})
            boolean extract,
            @Name({"-d", "--describe-module"})
            boolean describeModule,
            @Name({"-C"})
            @Cardinality(2)
            List<String> changeDir,
            @Name({"-f", "--file"})
        ...
        @Test
        void example2() {
            JarOptions options = parseInput("--create --date=\"2021-01-06T14:36:00+02:00\" --file=classes.jar Foo.class Bar.class");
            assertEquals(true, options.create());
        ...
        @Test
        void example4() {
            JarOptions options = parseInput("--create --file foo.jar --main-class com.foo.Main --module-version 1.0 -C foo/classes resources");
            assertEquals(true, options.create());
        ...
        @Test
        void example5() {
            JarOptions options = parseInput("--create --file foo.jar --main-class com.foo.Hello -C classes . --release 10 -C classes-10 .");
            assertEquals(true, options.create());
            assertEquals("foo.jar", options.file());
